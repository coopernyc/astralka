  ...
  template: `
    ...
    <ng-template cdkPortal #overlayTemplate="cdkPortal" class="dropdown">
      <cdk-virtual-scroll-viewport itemSize="26" minBufferPx="200" maxBufferPx="400" class="dd">
        <lookup-option [class.pub]="person.scope === PersonScope.Public" *cdkVirtualFor="let person of list;"
                       [value]="person" (selected)="selectOption($event)">
          <div class="dd-item">
            <div style="flex: 1; overflow: hidden;">{{ person.name }}</div>
            <div style="flex: 0 20px; text-align: right;">{{ person.scope === PersonScope.Private ? 'Prv' : 'Pub' }}
            </div>
          </div>
        </lookup-option>
